...
async function pollAll() {
  if (!config.insiderWallets.length) return;
  for (const wallet of config.insiderWallets) {
...
function start() {
  if (!config.insiderWallets.length) {
    logger.info('Wallet tracking not started (wallets.json has no insiderWallets - insider signal will read as absent, not required for entry)');
    return;
  }
  ...
  pollAll();
  setInterval(() => pollAll().catch((e) => logger.error('Wallet poll cycle failed', { error: e.message })), config.walletPollIntervalMs);
}

module.exports = { start, getBuyerCount, checkAndConsumeBigSell };
