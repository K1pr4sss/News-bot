...
/** Sizing tier off the entry score.
 *
 * The boundary is 60, not the original 55, because 60 is where the real ledger
 * splits. Across the 57 trades whose exits still exist in the current config:
 *
 *     score 0-50   n=29   mean -10.7%   win 31%   train  -4.7% / test -17.2%   both negative
 *     score 50-60  n=21   mean  -6.2%   win 38%   train  +7.3% / test -18.5%   flips
 *     score 60+    n= 7   mean +19.3%   win 86%   train +16.8% / test +21.1%   BOTH POSITIVE
 *
 * Six of those seven were profitable, spread across six separate days rather
 * than clustered in one lucky session. It is the only band that survives a
 * chronological split, and it survives it strongly.
 *
 * Size is the right lever here rather than a gate. Raising the ENTRY bar to 60
 * was tried on 2026-09-09 and produced zero trades in 24.8 hours, because band
 * coins top out near 50 - the score is built from volume spike and trending
 * presence, which the momentum band deliberately selects against. Gating on 60
 * starves the funnel; sizing on 60 concentrates capital into the trades that
 * actually work while the bar stays where candidates exist.
 */
function computeSizeTier(score) {
  if (score >= 70) return { pct: config.sizeTier3Pct, holdMinutes: config.holdMinutesTier3, label: '70+' };
  if (score >= 60) return { pct: config.sizeTier2Pct, holdMinutes: config.holdMinutesTier2, label: '60-70' };
  return { pct: config.sizeTier1Pct, holdMinutes: config.holdMinutesTier1, label: 'base' };
}
