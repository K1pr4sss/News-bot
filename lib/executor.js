...
class PaperExecutor {
  ...
  /** Simulated fill at the live price with a flat slippage/fee haircut. */
  async buy({ amountSol, priceUsd }) {
    const balance = this.getBalanceSol();
    if (amountSol > balance) return { success: false, error: 'insufficient paper balance' };
    const filledPriceUsd = priceUsd * (1 + config.paperSlippagePct / 100);
    this._setBalanceSol(balance - amountSol);
    logger.info('Paper BUY filled', { amountSol, filledPriceUsd });
  ...
  async sell({ amountSol, priceUsd, entryPriceUsd }) {
    ...
    const proceedsSol = amountSol * (1 + changeFraction);
    const balance = this.getBalanceSol();
    this._setBalanceSol(balance + proceedsSol);
    logger.info('Paper SELL filled', { amountSol, filledPriceUsd, proceedsSol });
