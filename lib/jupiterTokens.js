...
/**
 * The rug gate. The ONLY thing in this file that gates anything, and it earned
 * it by separating perfectly on the one question that actually costs money.
 *
 * Every position taken since this data started being recorded, with the organic
 * score READ AT ENTRY - before the outcome, so this is a prediction and not
 * hindsight:
 *
 *     HOOD       0.0  low       rugged  -99.9%
 *     DREGG      0.0  low       rugged -100.0%
 *     GOOG      38.3  medium    +40% take-profit
 *     KESTREL   48.4  medium    -1.0%
 *     SAAR      54.0  medium    -0.9%
 *     KINS      61.7  medium    -0.5%
 *     HeeHaw    63.8  medium    -1.9%
 *     RETARDIO  65.3  medium    -0.1%
 *     nub       66.8  medium    -0.5%
 *     fone      84.3  high      +0.2%
 *     Bonk      87.1  high      -0.1%
 *     STONK     95.6  high      +0.0%
 *
 * Two rugs, both at exactly zero. Ten survivors, none below 38. Both rugs also
 * returned no organic volume data at all.
 *
 * Those two trades were 80% of the loss over that window: -0.0990 and -0.0863
 * against -0.0475 for the other nine COMBINED. This is not an edge being
 * hunted, it is the tail that has been eating every edge the bot has found.
 *
 * Why act on n=12 when nothing else here was allowed to ship on small samples:
 * the payoff is violently asymmetric. A false positive skips one trade worth
 * about -0.5% in expectation. A false negative costs the ENTIRE position. At
 * that ratio the signal does not need to be strong to be worth using, it only
 * needs to be real - and the mechanism says it is, because organicScore exists
 * precisely to measure whether activity is genuine, and a coin about to rug has
 * none that is.
 *
 * The earlier finding that organicScore "does not predict" was measured across
 * 95 mints AFTER their outcomes, where a coin reads low BECAUSE it already died.
 * That test was contaminated in both directions and settled nothing either way.
 *
 * Threshold is the observed signature and barely more: Jupiter reporting
 * essentially zero organic activity. Deliberately not widened to the whole "low"
 * band, because there is no data between 0 and 38 and inventing a boundary there
 * is the overfitting this project keeps getting burned by.
 *
 * Fails OPEN on a missing reading, same rule as every other gate here.
 */
function isOrganicallyTradeable(jup) {
  if (!config.blockZeroOrganicScore) return { ok: true, reason: null };
  const score = jup?.organicScore;
  if (score === null || score === undefined) return { ok: true, reason: null };
  if (score < config.minOrganicScore) {
    return {
      ok: false,
      reason: `Jupiter organic score ${score} below ${config.minOrganicScore} `
        + '(no genuine activity detected - both rugs so far read exactly 0 at entry, no survivor read under 38)',
    };
  }
...
function getStatus() {
  return {
    ...status,
    configured: true,
    rugGate: config.blockZeroOrganicScore ? `organicScore >= ${config.minOrganicScore}` : 'off',
    cacheSize: cache.size,
  };
}

module.exports = {
  getTokenData, getDiscoveryCandidates, toCandidate, isOrganicallyTradeable, getStatus, EMPTY,
};
