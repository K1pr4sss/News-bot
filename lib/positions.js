...
async function attemptEntry(token, scoreResult) {
  ...
  try {
    ...
    db.prepare(`
      INSERT INTO positions (mint, name, symbol, entry_price_usd, original_amount_sol, remaining_amount_sol, entry_score, opened_at, max_hold_minutes, entry_socials_count, entry_trending_pool, entry_bonus_mentions)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      ...
      token.matchedTrendingPool ? 1 : 0, token.bonusMentionCount || 0,
    );
