...
// 'high' priority. These are the two calls that feed the ENTIRE funnel - one
// batched request each, regardless of how many tokens exist - while
// pendingCandidatesTick issues one speculative call per queued candidate and
// can legitimately want 25 of them in a tick. Left at equal priority, the
// speculative traffic crowds out discovery: observed live as repeated
// "getNewPools failed 429" while the pending queue kept churning, which starves
// the bot of new candidates entirely. Cheap calls that feed everything must
// outrank expensive calls that feed one candidate.
async function getNewPools() {
  try {
    const { data } = await throttledGet(`${BASE}/new_pools`, { timeout: 15000 }, 'high');
...
async function getTrendingPools(duration) {
  ...
  try {
    const { data } = await throttledGet(url, { timeout: 15000 }, 'high'); // see getNewPools
    return (data?.data || []).map(parsePool).filter(Boolean);
  } catch (err) {
