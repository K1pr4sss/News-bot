...
const config = {
  ...
  // RAISED 40 -> 60 on 2026-09-09. The only signal in this entire project that
  // is positive in BOTH chronological halves on real money.
  //
  // Across all 183 closed round trips:
  //     score >= 60   n= 28   mean  +0.9%   win 29%   total +0.0399 SOL
  //     score <  60   n=155   mean  -6.5%   win 14%   total -0.5152 SOL
  //
  // And it is a ramp, not a lucky cell - the cumulative thresholds improve
  // monotonically right up to it:
  //     >=50   n=77   -3.7%   (flips train/test)
  //     >=55   n=40   -2.1%   (both halves negative)
  //     >=60   n=28   +0.9%   (train +0.4% / test +3.8%  - BOTH POSITIVE)
  //
  // The mechanism matters more than the average. Reduced to arithmetic, this
  // strategy is: 17% of trades reach +40%, 17% stop out near -44%, and 67%
  // touch neither and time out roughly flat. At 2.4% friction that is -3.07%
  // per trade, and the ONLY ways out are a win rate above 25% or a stop rate
  // below 9%. Score >=60 wins 29% of the time. It clears the bar; everything
  // below it (14%) cannot.
  //
  // THE COST IS SEVERE AND DELIBERATE: only ~15% of candidates score this high
  // - in the band era it was 2 of 19 - so trade frequency drops by roughly
  // 85%. That is accepted rather than regretted. Trading a 14%-win signal more
  // often does not help; it loses faster. Ahead of real money, few and good
  // beats many and bad.
  //
  // Honest risk: n=28, several thresholds were examined, and >=65 falls back to
  // +0.1%. So 60 is the peak of a ramp rather than a plateau, which is weaker
  // than a flat neighbourhood would be. It is still the best-supported change
  // available, and unlike the last four candidates it survives the split.
  scoreAlertThreshold: Number(process.env.SCORE_ALERT_THRESHOLD || 60),
