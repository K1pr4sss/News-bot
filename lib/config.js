...
const config = {
  ...
  // --- Discovery budget, REBALANCED 2026-09-08 to match the strategy ---
  // The discovery layer was still built for the ORIGINAL strategy (snipe fresh
  // launches) while the entry logic now wants established tokens. That mismatch
  // was quietly strangling the funnel.
  //
  // Where band-era entries actually came from:
  //     trending poll   17 of 19
  //     new_pools poll   2 of 19
  //     pool age at entry: NOT ONE under 45 minutes; median 28 DAYS
  //
  // And where the polling budget was going:
  //     new_pools  every 20s  = 3.0 GeckoTerminal calls/min
  //     trending   every 5min = 0.6 calls/min
  //
  // Five times the budget spent on the source that produced two entries,
  // starving the one that produced seventeen. Downstream: 18,268 tokens
  // scanned in 17 hours, 10,732 evicted from a pending queue stuffed with
  // fresh pools that can never clear the age floor, and 3 buys.
  //
  // Inverted below, at the same total cost. new_pools is kept rather than
  // deleted because it is nearly free at this interval and it is the only
  // source that could ever catch a genuinely new token the trending lists have
  // not noticed - but it is no longer allowed to dominate the budget.
  discoveryPollIntervalMs: Number(process.env.DISCOVERY_POLL_INTERVAL_MS || 120 * 1000),
  ...
  // 5min -> 60s. This is the source that produces essentially every entry, so
  // it gets the budget that new_pools was wasting. One tick costs one call per
  // window in trendingDurations; at 60s with four windows that is 4 calls/min,
  // against a ~30/min ceiling.
  trendingPollIntervalMs: Number(process.env.TRENDING_POLL_INTERVAL_MS || 60 * 1000),

  // Jupiter's keyless token categories (toptrending + toporganicscore). The only
  // discovery source aimed at ESTABLISHED tokens, which is what the band buys -
  // everything else here looks at new pools. Measured against the 18,923 mints
  // the bot had processed over 43 hours, roughly half of each list was already
  // covered and the remainder was genuinely unseen, including coins the
  // new-pool feeds structurally cannot surface (wPOND: 702 days old, 6,752
  // holders). Two calls per tick; at three minutes that is 0.67 calls/min
  // against a limit that tolerates about 40.
  jupiterDiscoveryIntervalMs: Number(process.env.JUPITER_DISCOVERY_INTERVAL_MS || 3 * 60 * 1000),
  ...
  // 6h added. The band buys established tokens - median pool age at entry is 28
  // days - so the longer windows are where its candidates live, and 5m/1h/24h
  // left a gap between "running right now" and "nine days old".
  trendingDurations: (process.env.TRENDING_DURATIONS || '5m,1h,6h,24h')
  ...
  // --- Measured execution cost (see lib/jupiter.js) ---
  // The only gate in this bot fed by a MEASUREMENT rather than an inference,
  // and the most important consequence of that: it needs no train/test split.
  // A 7.8% round trip costs 7.8% whether or not the finding replicates.
  //
  // Every other cost figure in this file rests on paperSlippagePct - a flat 1%
  // per leg, the same for every coin. Quoting 12 real band-era holdings through
  // Jupiter both ways (SOL in, then those exact tokens straight back out) gives
  // the true cost including LP fees, price impact and routing on both legs:
  //
  ...
  //     moonkey 1.48%
  //
  // The median is 2.02%, so the 2% assumption was RIGHT - worth stating because
  // the hypothesis going in was that it was too pessimistic, and it was not.
  // What the assumption missed is the spread: p90 is 5.90%. The whole edge this
  // bot is fighting for is about 2.8% gross, so a coin costing 7.8% to
  // round-trip cannot win regardless of what its price then does. It was taking
  // those trades blind, and LOOM - the most expensive coin measured - was also
  // the worst loser of the band era.
  //
  // 3.0% keeps 9 of those 12 and excludes LOOM, ZCAT and HOOD, which between
  // them were 39% of the era's losses on 25% of its trades. Set to 0 to disable
  // the gate entirely (the quote is then skipped, saving the call).
  //
  // A missing reading is UNKNOWN, never a refusal - same rule as the momentum
  // gate failing open on an absent h1. A Jupiter outage degrades this bot to
  // the blind behaviour it had before; it must never halt it.
  //
  // RAISED 3.0 -> 5.0 on 2026-09-09, because 3.0 was measurably wrong. Tested
  // against the bot's OWN 19 band trades rather than the counterfactual, and it
  // blocked the best trade in the set while keeping the second worst:
  //
  //     HAPPYCAT  cost 3.11%   result +43.0%   <- BLOCKED by a 3% ceiling
  //     TSUKI     cost 0.51%   result -19.1%   <- passed
  //
  //     ceiling   kept mean    blocks
  //       3%       -4.04%      USEFUL, LOOM, ZCAT, HOOD, HAPPYCAT
  //       5%       -2.29%      LOOM, ZCAT
  //      none      -3.16%      -
  //
  // Worse, the gate has no predictive value at all here: Spearman rank
  // correlation between measured cost and realised result is 0.095, and the
  // cheapest ten trades averaged -6.53% against +0.59% for the dearest nine.
  // Cost does not rank outcomes. The correlation that looked so strong on 94
  // historical mints was the reverse causality already documented above - dead
  // coins quote expensive BECAUSE they died.
  //
  // So the gate keeps exactly the justification it was shipped with and nothing
  // more: pure arithmetic on the UNWINNABLE. Gross edge is at best ~3%, so a
  // coin costing 6-8% to round-trip cannot be won back whatever the price does.
  // LOOM at 7.79% and ZCAT at 6.20% are that case. HAPPYCAT at 3.11% never was,
  // and a 3% ceiling was quietly enforcing a prediction the data does not
  // support instead of the arithmetic it was meant to enforce.
  //
  // 4% scored best in the sweep (-1.04%). Not taken: that is the best of eight
  // cells on n=19, which is the overfit this project has already been burned by
  // four times. 5% is where the arithmetic argument actually lands.
  maxRoundTripCostPct: Number(process.env.MAX_ROUND_TRIP_COST_PCT || 5.0),

  // --- Telegram override ---
  // A coin named in a tracked alpha group skips the MOMENTUM BAND and nothing
  // else. This is an EXPERIMENT, and the distinction matters because every
  // other gate in this file is backed by a measurement and this one is not.
  //
  // The problem it addresses is real and measured. The momentum ceiling blocks
  // 100% of coins up more than 50% in an hour - 4,581 of them in 43 hours - so
  // the bot structurally cannot participate in any hype wave. That ceiling
  // exists because high-momentum coins measured badly (-5.9% race EV, 39%
  // falling below -80%), which is solid: 137 real trades over 8 days.
  //
  // But that measurement lumped every pump together, and the bot has no way to
  // tell a real one from a manufactured one. Its score is built from volume
  // spike (25 pts) and trending presence (20 pts) - both ON-CHAIN - so a
  // wash-traded pump scores exactly like a genuine one. Given that 82.89% of
  // high-return memecoins show artificial growth, refusing all pumps was the
  // right call while "is this real?" was unanswerable.
  //
  // A named call in a curated alpha group is a signal from OFF-chain, which is
  // the one place a wash trader cannot manufacture cheaply. It is the only
  // input available that could separate the two.
  //
  // Why it is safe enough to try:
  //   - frequency is tiny. Those groups post a few times a day and hold ~2
  //     messages at a time, so this fires rarely by construction.
  //   - it waives ONE judgement ("has this moved too much"), not safety. Rug
  //     verdict, mint/freeze authority, liquidity floor, top-holder cap,
  //     insider clustering, pool age, churn ceiling and the measured
  //     execution-cost gate all still apply.
  //   - position size is unchanged at 5%.
  //   - entries taken this way are tagged, so their performance is measurable
  //     on its own rather than blended into the band's.
  //
  // Set to false to disable. If the tagged entries do not outperform, they
  // should be turned off rather than explained.
  telegramOverridesMomentum: process.env.TELEGRAM_OVERRIDES_MOMENTUM !== 'false',

  // --- Rug gate (see lib/jupiterTokens.isOrganicallyTradeable) ---
  // Refuse a coin Jupiter reports as having essentially no genuine activity.
  // Both rugs the bot has taken since this reading started being recorded came
  // in at exactly 0, and no surviving position read under 38 - all measured AT
  // ENTRY, before the outcome. Those two trades were 80% of that window's loss
  // (-0.0990 and -0.0863, against -0.0475 for the other nine combined).
  //
  // Shipped on n=12 deliberately, against this file's usual standard, because
  // the payoff is asymmetric to the point where the usual standard is the wrong
  // one: a false positive skips a trade worth about -0.5% in expectation, a
  // false negative costs the whole position. The signal does not need to be
  // strong at that ratio, only real.
  //
  // 1.0 rather than the whole "low" band: there is no observed data between 0
  // and 38, and drawing a boundary inside a gap is exactly the overfitting that
  // has produced four false findings here already.
  minOrganicScore: Number(process.env.MIN_ORGANIC_SCORE || 1.0),
  blockZeroOrganicScore: process.env.BLOCK_ZERO_ORGANIC_SCORE !== 'false',

  // --- Scoring (spec Section 5) ---
  // 60 -> 44, correcting a threshold I derived from the WRONG POPULATION and
  // which produced ZERO trades in 24.8 hours.
  //
  // The 60 bar came from all 183 historical trades, where score >=60 won 29%
  // against 14% below it. But almost every one of those trades was made under
  // the OLD momentum gate (h1 >= 50%) - hot coins, which score high because the
  // score is largely built from volume spike and trending presence. Then the
  // momentum gate was inverted to select CALM coins (h1 1-15%), and calm coins
  // score far lower by construction:
  //
  //     band coins  (h1 1-15%)   n=364   median 20   MAX 50   reaching 60: 0
  //     hot coins   (h1 > 50%)   n=229   median 35   MAX 55   reaching 60: 0
  //
  // A threshold measured on one population, applied to a different one. That is
  // the exact error this file warns about repeatedly, and I made it. 23,000
  // tokens scanned, 0 alerts, 0 trades.
  //
  // Re-derived on the 20 trades actually made under the band, which is the only
  // population that is live:
  //
  //     bar   kept   mean     win%    total SOL
  //     >=40    20   -1.0%     25%    -0.0198
  //     >=43    18   +0.4%     28%    +0.0052
  //     >=44    13   +5.0%     38%    +0.0606   <- here
  //     >=46     9   +7.2%     44%    +0.0604
  //     >=49     8   +8.6%     50%    +0.0640
  //     >=53     7  +10.3%     57%    +0.0675
  //     >=60     2  +23.7%    100%    +0.0430
  //
  // Monotone at every step, which is a shape rather than a lucky cell - and a
  // stronger basis than the 60 finding ever had. 44 is chosen over the higher
  // (better-looking) cells deliberately: it keeps 65% of candidates instead of
  // 10%, its 38% win rate already clears the ~25% needed to break even at this
  // payoff, and picking the maximum of a sweep on n=20 is how the last four
  // false findings happened.
  //
  // KNOWN STRUCTURAL TENSION, not fixed here: the score is built mostly from
  // volume spike (25) and trending presence (20), so it rewards precisely the
  // hot profile the momentum band now rejects. The band cannot score highly on
  // the current formula. Re-weighting it is the right fix, but it moves the
  // scale again and would invalidate this threshold too, so it needs forward
  // data first - which is what rawMentionCount, telegramMentions8h and the
  // Jupiter token fields are being recorded for.
  scoreAlertThreshold: Number(process.env.SCORE_ALERT_THRESHOLD || 44),
  ...
  // UN-FLATTENED, at a boundary of 60. These were flattened on 2026-09-07 on the
  // finding that the score was anti-predictive - measured across 45 trades with
  // replayed forward candles, where the higher band was the worse one. That
  // measurement has since been superseded by the real ledger, which disagrees,
  // and the ledger wins: across the 57 trades whose exits still exist,
  //
  //     score 0-50   n=29   mean -10.7%   win 31%   train  -4.7% / test -17.2%
  //     score 50-60  n=21   mean  -6.2%   win 38%   train  +7.3% / test -18.5%
  //     score 60+    n= 7   mean +19.3%   win 86%   train +16.8% / test +21.1%
  //
  // Six of the seven 60+ trades were profitable, spread across six separate
  // days. It is the only band positive in both halves, and strongly so.
  //
  // SIZE, not a gate. Raising the entry bar to 60 was tried and produced zero
  // trades in 24.8 hours, because band coins top out near 50 - the score is
  // built from volume spike and trending presence, which the momentum band
  // selects against. Gating on 60 starves the funnel; sizing on 60 puts more
  // capital into the trades that work while the bar stays where candidates are.
  //
  // 0.08 rather than anything Kelly-shaped: the 60+ subset computes to a
  // full-Kelly of about 85%, which is what Kelly always says on seven samples
  // with one small loss. 1.6x the base is a deliberate under-bet of a number
  // that cannot be trusted at this n.
  sizeTier2Pct: Number(process.env.SIZE_TIER2_PCT || 0.08), // score 60-70
  sizeTier3Pct: Number(process.env.SIZE_TIER3_PCT || 0.08), // score 70+ - same, no evidence separates them
  // Raised in step, or the tiers above would be silently clamped back to 5%.
  maxTradePct: Number(process.env.MAX_TRADE_PCT || 0.08),
  ...
  // Back to 5. This was lowered to 3 to hold total exposure flat when
  // sizeTier1Pct went to 8% (5 x 5% = 25% before, 3 x 8% = 24% then). Sizing
  // has since been cut back to 5%, so 3 slots is 15% deployed - well under the
  // ~25% the cap was calibrated against, and under-deployed against its own
  // design rather than deliberately conservative.
  //
  // It is also demonstrably binding: 8 entries were refused with "max open
  // positions reached" in a single 20-hour window, on a bot whose main problem
  // is too few trades to learn from. 5 x 5% restores the original 25%.
  //
  // The exit-tick argument for keeping it low no longer applies either -
  // exitTick is price-only and runs its positions in parallel, so five costs
  // the same wall time as three.
  ...
  //
  // CORRECTED 2026-09-08, from 0.001, and worth recording HOW rather than just
  // that - because this exact change was refused a few hours earlier.
  //
  // It was refused then on the grounds that cutting a cost assumption to reach
  // profitability is fitting, not analysis. That was right at the time: there
  // was a hypothesis that 0.001 looked high, and no measurement. There is now a
  // measurement, from getRecentPrioritizationFees on a public Solana RPC over
  // 150 recent slots:
  //
  //     priority fee (micro-lamports per compute unit)
  //       p10 0    median 0    p75 34,124    p90 144,060
  //
  //   all-in cost of a 150k-300k CU swap, including the 5,000 lamport base fee:
  //       median        5,000 lamports = 0.000005 SOL
  //       p75      10,000-15,000       = 0.000015 SOL
  //       p90      27,000-48,000       = 0.000048 SOL
  //       modelled  1,000,000          = 0.001    SOL   <- 20x to 200x high
  //
  // The 0.002-0.0025 figure this inherited came from the SNIPER bot, and that
  // bot's number was correct FOR IT: sniping a brand-new launch is a race for
  // block position, so you outbid rivals or you miss the fill. This bot buys
  // pools 45+ minutes old inside a mild-momentum band. Nothing is being raced,
  // nothing needs outbidding, and landing a slot or two later is irrelevant to
  // a two-hour trade. The old value was not wrong, it was borrowed from a
  // different problem.
  //
  // 0.0002 is deliberately 4x the measured p90 - covering retries (a failed
  // Solana transaction still pays), congestion spikes, and being wrong - while
  // still sitting 5x below what was modelled.
  //
  // What it changes, at the 3% cost gate, on the band's real paths with each
  // coin charged its own measured trading cost:
  //     fee/leg     fee share    EV/trade    train     test
  //     0.001         2.00%       +0.12%     -0.39%   +0.60%   (flips)
  //     0.0002        0.40%       +1.72%     +1.21%   +2.20%   (both positive)
  //
  // And it removes the account-size problem entirely across the range that is
  // actually planned for live money - 1 SOL +1.32%, 2 SOL +1.72%, 3 SOL +1.86%,
  // all positive in both halves. Raising the balance was the previous answer to
  // that gap; measuring the fee is a better one, because it does not require
  // testing at a size that will never be deployed.
  paperFeeSol: Number(process.env.PAPER_FEE_SOL || 0.0002),
  ...
  // Qualify the YouTube query with crypto context instead of searching the bare
  // coin name. The bare name was searching for an English WORD, not a coin:
  // "HOOD", "fone", "GOOG", "LAPTOP" each returned the full 50 results of
  // entirely unrelated video, every single time - 18 calls produced 900
  // "mentions", exactly 50 per call.
  //
  // That was not merely noise in the score. evaluator.js adds this count to
  // mentionCount BEFORE the minMentionCount gate, so "at least one real mention"
  // was being satisfied for essentially every candidate by videos about laptops.
  // The hype gate on a hype bot was not gating.
  //
  // Watch youtube.zeroResultSearches on /sources after this ships. If it climbs
  // to match totalCalls the query is too strict and this flips to false - the
  // risk being that a genuinely hyped coin has no YouTube coverage at all, in
  // which case the gate starves the funnel the way the score bar at 60 did.
  youtubeQualifySearch: process.env.YOUTUBE_QUALIFY_SEARCH !== 'false',
