...
const config = {
  ...
  // Raised from 0.05. Not to take more risk per se - because the FLAT fee is
  // fixed at paperFeeSol while position size is a % of a falling balance, which
  // makes friction a ratchet: 6.0% of position at a 1.0 SOL balance, 8.6% at
  // 0.61, 15.3% at 0.30, 22% at 0.20. Every loss makes the next trade
  // structurally harder to win.
  //
  // Replaying the 108 real positions with only the size changed and every
  // decision held constant, ROI rises with size - 1.35% at x1, 2.24% at x2,
  // 2.54% at x3 - which is direct evidence the flat fee was consuming real
  // edge rather than there being no edge. Bigger positions do not CREATE an
  // edge; they stop a fixed cost from eating one.
  //
  // Deliberately modest (0.05 -> 0.08 rather than doubling) and paired with a
  // lower maxOpenPositions, so total exposure barely moves: it was 5 x 5% =
  // 25%, it is now 3 x 8% = 24%. This is a friction change, not a leverage one.
  sizeTier1Pct: Number(process.env.SIZE_TIER1_PCT || 0.12), // score 40-55
  // The flat fee makes small positions structurally unprofitable, and no entry
  // or exit rule can fix that - it is arithmetic. Round-trip friction by size,
  // at 2 legs:
  //     0.030 SOL -> 8.7%      0.048 SOL -> 6.2% (what it was running)
  //     0.080 SOL -> 4.5%      0.100 SOL -> 4.0%
  // The same +40/-35 config earns test EV +2.1% at 0.048 SOL and +4.3% at
  // 0.10 SOL. Identical decisions, identical prices - only the fee share
  // differs. So a trade too small to clear its own costs should not be taken
  // at all, rather than taken and lost slowly.
  //
  // This also protects against the death-spiral: sizing is a % of a falling
  // balance, so a shrinking account would otherwise keep taking ever-smaller
  // trades at ever-worse friction until nothing could win. Below this floor the
  // bot stops trading instead, which is the correct response to an account too
  // small to overcome its own fixed costs.
  minPositionSol: Number(process.env.MIN_POSITION_SOL || 0.05),
