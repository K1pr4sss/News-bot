...
// The measured round-trip cost of the position, in percent, from lib/jupiter.js.
// Stored as its own column rather than only inside entry_metrics because the
// SELL side needs it on every exit tick, and parsing a JSON blob per tick to
// price a fill is the wrong shape. Null means the cost was never established,
// and the flat assumption applies.
const positionColumns2 = db.prepare('PRAGMA table_info(positions)').all().map((c) => c.name);
if (!positionColumns2.includes('entry_round_trip_cost_pct')) {
  db.exec('ALTER TABLE positions ADD COLUMN entry_round_trip_cost_pct REAL');
  logger.info('Migrated positions table: added entry_round_trip_cost_pct column');
}

// The rejections table already exists on the live volume, so CREATE TABLE IF
// NOT EXISTS above is a no-op there - these columns need the same explicit
// migration treatment as the positions ones. All three are pump.fun fields the
// bot was already fetching and discarding; recorded now so the "is a coin at
// its all-time high a bad buy?" question becomes measurable rather than a
// plausible story. See pumpfunApi.getSocials.
const rejectionColumns = db.prepare('PRAGMA table_info(rejections)').all().map((c) => c.name);
//
// pool_age_minutes and rugcheck_score added 2026-09-07 for a specific reason:
// the pool-age gate is the strongest signal found so far, and it could not be
// checked against the counterfactual population (coins the bot rejected)
// because rejections never recorded an age. rugcheck_score is the same story -
// RugCheck returns score_normalised on every call the bot already makes, and
// it has been thrown away every time. Recorded, not gated, so both become
// answerable rather than arguable.
//
// mention_count / telegram_mentions added 2026-09-10 to answer a question the
// data currently cannot: the momentum ceiling blocks 100% of coins up more than
// 50% in an hour - 4,581 of them in 43 hours - because high momentum measured
// badly. But that measurement lumped every pump together, and the score is
// built from volume spike and trending presence, both ON-CHAIN, so a
// wash-traded pump scores exactly like a real one. Whether a pump with genuine
// social behind it behaves differently has never been testable. Telegram,
// Reddit and Google Alerts are read for EVERY candidate (before the filters),
// so recording them on rejections makes it testable.
for (const [col, type] of [['off_ath_pct', 'REAL'], ['usd_market_cap', 'REAL'], ['reply_count', 'INTEGER'],
  ['pool_age_minutes', 'REAL'], ['rugcheck_score', 'INTEGER'],
  ['mention_count', 'INTEGER'], ['telegram_mentions', 'INTEGER']]) {
  if (!rejectionColumns.includes(col)) {
    db.exec(`ALTER TABLE rejections ADD COLUMN ${col} ${type}`);
    logger.info(`Migrated rejections table: added ${col} column`);
  }
}
...
// One-off top-up to 2 SOL, keyed in meta so a redeploy can never repeat it.
//
// Not a cosmetic reset. Position size is a percentage of the balance while the
// fee is FLAT, so a small balance makes friction dominate everything: at the
// 0.021 SOL position a 0.44 balance supports, a round trip costs 11.5% before
// the strategy does anything. The validated entry rule is worth about +5.4%
// raw, so at that size a genuinely positive edge still lands at -6.1%.
//
// Raising the size FRACTION instead is the trap that produced the ruin
// arithmetic in config.sizeTier1Pct - 12% of the observed return distribution
// bootstraps to a 100% chance of losing 90% of the account. 5% of 2 SOL gives
// 0.10 SOL positions at 4.0% friction, which is what a real trader on Solana
// actually faces, and it is the only way to measure the entry change rather
// than measure the fee.
//
// The trades table is untouched: every historical fill, P&L figure and the
// -0.39 SOL realised total remain exactly as they were. This resets the
// forward experiment's stake, not its record.
const TOPUP_KEY = 'paper_balance_topup_2sol_2026_09_07';
if (existingWallet && !getMeta(TOPUP_KEY)) {
  const before = db.prepare('SELECT balance_sol FROM paper_wallet WHERE id = 1').get().balance_sol;
  db.prepare('UPDATE paper_wallet SET balance_sol = ? WHERE id = 1').run(2.0);
  setMeta(TOPUP_KEY, new Date().toISOString());
  logger.info('Paper wallet topped up for the forward test', { before, after: 2.0 });
}
