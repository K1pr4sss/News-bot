...
async function start() {
  ...
  try {
    ...
    try {
      ...
      const dialogNames = dialogs.map((d) => ({ title: d.title, username: d.entity?.username || null }));
      logger.info('Telegram account dialog list fetched', { count: dialogNames.length });
      for (const g of trackedGroups) {
        const found = dialogNames.some(
          (d) => d.title?.toLowerCase() === g.toLowerCase() || d.username?.toLowerCase() === g.toLowerCase(),
        );
        if (found) {
          logger.info('Tracked group confirmed - account is still a member', { group: g });
        } else {
