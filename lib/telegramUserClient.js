...
async function start() {
  ...
  try {
    ...
    // Real bug found live (2026-09-03): watched real-time debug logs for
    // 6+ minutes while the user confirmed multiple tracked channels were
    // actively posting - captured zero channel events the whole time (only
    // the account's own private DM chat fired). This is a documented,
    // long-standing GramJS/MTProto quirk, not something specific to this
    // bot: channel update delivery relies on per-channel pts tracking that
    // silently goes idle without periodic active requests, while private-
    // chat updates keep flowing fine on the same connection - matches
    // exactly what was observed (see gram-js/gramjs issues #280, #535,
    // #682, #685 - the library itself is now archived/unmaintained, so this
    // is a permanent characteristic to work around, not a bug that'll get
    // fixed upstream). The documented community workaround is a periodic
    // lightweight authenticated call to keep the update stream alive -
    // getMe() costs nothing and touches no chat state.
    setInterval(() => {
      client.getMe().catch((err) => logger.debug('Telegram keep-alive call failed', { error: err.message }));
    }, 4 * 60 * 1000);
  } catch (err) {
