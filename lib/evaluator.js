...
async function evaluateCandidate(token, { trendingPool = false } = {}) {
  ...
  enriched.mentionCount = redditSignal.mentionCount + alertsSignal.mentionCount;
  enriched.positiveRatio = redditSignal.positiveRatio; // Alerts headlines are too sparse for a meaningful sentiment read - Reddit stays the only sentiment source
...
async function getLiveTokenAndScore(position) {
  ...
  const enriched = {
    ...
    mentionCount: redditSignal.mentionCount + alertsSignal.mentionCount,
