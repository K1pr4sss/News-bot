...
const jupiterTokens = require('./jupiterTokens');
...
function isPermanentReason(reason) {
  return reason.startsWith('copycat')
    ...
    // "pool only Nmin old" IS fixable by time in principle, and was queued on
    // that reasoning. In practice it is the single worst thing to queue. A
    // pool five minutes old needs forty more to clear the floor, and the queue
    // evicted 10,732 candidates in seventeen hours - nothing survives forty
    // minutes in it. So every fresh pool took a slot it could never cash in,
    // starving the candidates whose momentum genuinely does flip minute to
    // minute, which is what the queue exists for.
    //
    // Dropping them costs nothing real: 17 of the 19 band-era entries came
    // from the TRENDING poll and not one was under 45 minutes old (median pool
    // age at entry: 28 days). Established tokens are re-found continuously by
    // trending. A pool that matters will come back when it is old enough AND
    // actually moving; it does not need a seat held for it.
    || reason.startsWith('pool only');
}
...
async function evaluateCandidate(token, { trendingPool = false, isRetry = false } = {}) {
  ...
  const enriched = {
    ...
    // Carried through so both the rejections table and entry_metrics record it.
    // Recorded, NOT gated on - see pumpfunApi.getSocials.
    offAthPct: socials.offAthPct,
    usdMarketCap: socials.usdMarketCap,
    replyCount: socials.replyCount,
    bondingCurveComplete: socials.bondingCurveComplete,
    // RugCheck's own aggregate risk score, returned on every call the bot was
    // already making and discarded every time. Carried so entry_metrics records
    // it alongside the rejections table, which is what makes it testable
    // against outcomes later. Recorded, not gated.
    rugcheckScore: rugcheckReport.scoreNormalised,
    topHolderPct: rugcheckReport.topHolderPct,
    insiderNetworkPct: rugcheckReport.insiderNetworkPct,
  ...
  // Named in a tracked alpha group. Read over the full retention window rather
  // than the 5-minute velocity window, because a call is not a burst - it stays
  // relevant for hours. See config.telegramOverridesMomentum.
  enriched.telegramCalled = telegramUserClient.getSignal(token.symbol || token.name, 8 * 60).mentionCount > 0;
  ...
  if (pass && scoreResult.volumeRatio >= config.entryVolumeSpikeMultiplier && scoreResult.score >= config.scoreAlertThreshold - 10) {
    ...
    // The uncapped YouTube figure, carried through for RECORDING only - it never
    // touches the score. See lib/youtube.js for why the scored value stays
    // capped at 5 while the real number is collected alongside it.
    enriched.rawMentionCount = (youtubeSignal.rawMentionCount ?? youtubeSignal.mentionCount ?? 0)
      + twitterSignal.mentionCount + farcasterSignal.mentionCount;
    // Telegram read over a MUCH wider window, recorded and not scored. The
    // scored read uses socialMentionWindowMinutes (5), which treats a Telegram
    // post as a velocity signal - but an alpha-group call is not a burst, it is
    // a call, and it stays relevant for far longer than five minutes. YouTube
    // already uses 24 hours for exactly this reason. Recording the hour-long
    // read is what makes it possible to check that intuition against outcomes
    // before letting it move any score.
    enriched.telegramMentions1h = telegramUserClient.getSignal(token.symbol || token.name, 60).mentionCount;
    // And wider still. The tracked groups post a few times a day, so even an
    // hour is narrow for them - see RETENTION_MS in telegramUserClient.
    enriched.telegramMentions8h = telegramUserClient.getSignal(token.symbol || token.name, 8 * 60).mentionCount;
    if (bonusMentions > 0) {
    ...
    try {
      db.prepare(`
        INSERT INTO rejections (mint, symbol, reasons, sole_reason, top_holder_pct, insider_network_pct,
          price_change_h1_pct, price_change_m5_pct, liquidity_usd, volume_h1_usd, buyers_h1, sellers_h1, score,
          off_ath_pct, usd_market_cap, reply_count, pool_age_minutes, rugcheck_score,
          mention_count, telegram_mentions, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        ...
        scoreResult.score,
        enriched.offAthPct ?? null, enriched.usdMarketCap ?? null, enriched.replyCount ?? null,
        token.poolCreatedAt ? (Date.now() - token.poolCreatedAt) / 60000 : null,
        Number.isFinite(rugcheckReport.scoreNormalised) ? Math.round(rugcheckReport.scoreNormalised) : null,
        enriched.mentionCount ?? null,
        telegramSignal.mentionCount ?? null,
        Date.now(),
      );
    } catch (err) {
  ...
  // Jupiter token intel, fetched HERE rather than alongside the other
  // enrichment. It was running for every candidate, and adding Jupiter as a
  // discovery source took the scan rate from 16/min to 56/min - which would
  // have meant ~56 calls/min into a rate limit that tolerates roughly 40, on a
  // field that gates nothing. Fetching only for candidates that have cleared
  // every filter costs a handful of calls a minute and keeps the reading where
  // it actually matters: on the rows that become positions.
  enriched.jupToken = await jupiterTokens.getTokenData(token.mint);

  // The rug gate. Sits here rather than in runSafetyFilters because it needs
  // the Jupiter fetch above, which only runs for candidates that already
  // cleared everything cheaper. See jupiterTokens.isOrganicallyTradeable for
  // the two trades that justify it.
  const organicCheck = jupiterTokens.isOrganicallyTradeable(enriched.jupToken);
  if (!organicCheck.ok) {
    stats.recordRejection([`organic score ${enriched.jupToken.organicScore} - no genuine activity`]);
    logger.info('Entry refused by the rug gate', {
      mint: token.mint, symbol: enriched.symbol, organicScore: enriched.jupToken.organicScore,
    });
    return;
  }
  ...
  if (entry.ok) {
    ...
    // A candidate that got this far cleared every filter AND scored high enough
    // to alert - the strategy WANTED it. Refusals here are a different and more
    // urgent signal than a filter rejection, and until now they were visible
    // only in a Telegram message nobody diffs. See stats.recordEntryFailure.
    stats.recordEntryFailure({ symbol: enriched.symbol, mint: token.mint, reason: entry.reason });
  }
...
module.exports = {
  ...
  // exported for the queue-admission tests: which rejection reasons time can fix
  isPermanentReason,
};
