...
async function evaluateCandidate(token, { trendingPool = false, isRetry = false } = {}) {
  ...
  const [rugcheckReport, socials, birdeyeOverview, jupTokenData] = await Promise.all([
    rugcheck.getFullReport(token.mint),
    pumpfunApi.getSocials(token.mint),
    birdeye.getTokenOverview(token.mint),
    // Recorded, never gated - see lib/jupiterTokens.js. Runs in the same
    // Promise.all as the rest so it adds no wall time to the pipeline.
    jupiterTokens.getTokenData(token.mint),
  ]);
