...
async function evaluateCandidate(token, { trendingPool = false } = {}) {
  ...
  const youtubeSignal = youtube.getSignal(token.symbol || token.name);
  enriched.mentionCount = redditSignal.mentionCount + alertsSignal.mentionCount + telegramSignal.mentionCount + youtubeSignal.mentionCount;
...
async function getLiveTokenAndScore(position) {
  ...
  const youtubeSignal = youtube.getSignal(position.symbol || position.name);
  const enriched = {
    ...
    mentionCount: redditSignal.mentionCount + alertsSignal.mentionCount + telegramSignal.mentionCount + youtubeSignal.mentionCount,
