...
async function evaluateCandidate(token, { trendingPool = false, isRetry = false } = {}) {
  ...
  // Collected incrementally as each source is actually checked (some only
  // run conditionally below), then rendered into the alert's "Why:" line.
  const mentionSourceNames = [];
  if (redditSignal.mentionCount > 0) mentionSourceNames.push('Reddit');
  if (alertsSignal.mentionCount > 0) mentionSourceNames.push('Google Alerts');
  if (telegramSignal.mentionCount > 0) mentionSourceNames.push('Telegram alpha groups');
  ...
  if (pass && scoreResult.volumeRatio >= config.entryVolumeSpikeMultiplier && scoreResult.score >= config.scoreAlertThreshold - 10) {
    const [youtubeMentions, twitterMentions, farcasterSignal] = await Promise.all([
    ...
    if (youtubeMentions > 0) mentionSourceNames.push('YouTube');
    if (twitterMentions > 0) mentionSourceNames.push('X/Twitter');
    if (farcasterSignal.mentionCount > 0) mentionSourceNames.push('Farcaster');
    const bonusMentions = youtubeMentions + twitterMentions + farcasterSignal.mentionCount;
    if (bonusMentions > 0) {
