...
function start() {
  ...
  onText(/^\/check\s+(\S+)(?:\s+(\S+))?/, async (msg, match) => {
    ...
    if (term) {
      ...
      const [youtubeCount, twitterCount, farcasterSignal] = await Promise.all([
        youtube.searchMentionCount(term).catch(() => 0),
        twitter.searchMentionCount(term, 60).catch(() => 0),
        farcaster.getSignal(term, 60).catch(() => ({ mentionCount: 0 })),
      ]);
      lines.push(
        '',
        `Mentions in last 60min for "${term}" (proves each source is actually returning data, not just connected):`,
        `  Reddit: ${redditSignal.mentionCount}${config.redditClientId ? '' : ' (not configured)'}`,
        `  Google Alerts: ${alertsSignal.mentionCount}${config.googleAlertsRssUrls.length ? '' : ' (not configured)'}`,
        `  Telegram groups: ${telegramSignal.mentionCount}${telegramUserClient.getTrackedGroups().length ? '' : ' (no groups tracked)'}`,
        `  YouTube (last 24h): ${youtubeCount}${config.youtubeApiKey ? '' : ' (not configured)'}`,
        `  X/Twitter, unofficial (uses real daily budget): ${twitterCount}${config.getxapiApiKey ? '' : ' (not configured)'}`,
        `  Farcaster: ${farcasterSignal.mentionCount}${config.neynarApiKey ? '' : ' (not configured)'}`,
      );
    } else {
